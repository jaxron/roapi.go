...
type UserPresenceResponse struct {
	...
	PlaceID          *int64           `json:"placeId"          validate:"omitempty,min=1"` // ID of the place if user is in game
	RootPlaceID      *int64           `json:"rootPlaceId"      validate:"omitempty,min=1"` // ID of the root place if user is in game
	GameID           *string          `json:"gameId"           validate:"omitempty,min=1"` // ID of the game instance if user is in game
	UniverseID       *int64           `json:"universeId"       validate:"omitempty,min=1"` // ID of the universe if user is in game
	UserID           int64            `json:"userId"           validate:"required,min=1"`  // ID of the user
